...
async def get_current_user(
    ...
    try:
        ...
        token_data = am.AuthTokenContent(
            username=username, scopes=token_scopes, client_device_user_id=token_cduid
        )
    except JWTError:
        raise credentials_exception
...
def register(router):
    ...
    async def login(
        ...
        client_device = db.op.get_client_device_by_reported_name(
            device_name=device_name
        )
        ...
        client_device_user = db.op.get_client_device_user_by_ids(
            client_device_id=client_device.id, snowgroove_user_id=user.id
        )
