...
def handle(scope):
    db.op.update_job(
        job_id=scope.job_id, message=f'[WORKER] Handling an apply_directory_tag job'
    )

    if not scope or not scope.is_directory():
        db.op.update_job(
            job_id=scope.job_id,
            message='apply_directory_tag must be scoped to a directory',
        )
        return False

    db.op.update_job(
        job_id=scope.job_id,
        message=f'Applying tag [{scope.target_id}] to directory [{scope.target_directory}]',
    )
    ...
    for movie in movies:
        ...
        if scope.update_metadata:
            db.op.update_job(
                job_id=scope.job_id,
                message=f'Movie ({movie_count}/{movie_max}) {movie.directory}',
            )
            nfo_dict = snow_media.nfo.nfo_xml_to_dict(
                metadata.xml_content, step_into_root=False
            )
            ...
            db.op.update_metadata_file_content(
                metadata_file_id=metadata.id, xml_content=nfo_xml
            )
            snow_media.nfo.save_xml_as_nfo(
                nfo_path=metadata.local_path, nfo_xml=nfo_xml
            )
            ...
            db.op.update_job(
                job_id=scope.job_id,
                message=f'Movie ({movie_count}/{movie_max}) [DRY] {movie.directory}',
            )
        movie_count += 1
    ...
    for show in shows:
        ...
        if scope.update_metadata:
            db.op.update_job(
                job_id=scope.job_id,
                message=f'Show ({show_count}/{show_max}) {show.directory}',
            )
            nfo_dict = snow_media.nfo.nfo_xml_to_dict(
                metadata.xml_content, step_into_root=False
            )
            ...
            db.op.update_job(
                job_id=scope.job_id,
                message=f'Show ({show_count}/{show_max}) [DRY] {show.directory}',
            )
        show_count += 1
    ...
    if not shows and not movies:
        db.op.update_job(
            job_id=scope.job_id,
            message=f'No shows nor movies found in {scope.target_directory}',
        )
        return False
