...
def prep(files, movie=None, show=None, show_season=None, show_episode=None):
    ...
    for ff in files:
        ...
        if show_season:
            result.show_season = db.Box()
            result.show_season.id = show_season.id
        if show_episode:
            result.show_episode = db.Box()
...
def handle(scope):
    db.op.update_job(
        job_id=scope.job_id, message=f'[WORKER] Handling a read_media_files job'
    )
    metadata_files = None
    image_files = None
    video_files = None
    ...
    if (
        not scope.update_metadata
        and not scope.update_videos
        and not scope.update_images
    ):
        db.op.update_job(
            job_id=scope.job_id,
            message='read_media_files requires update_videos, update_images, or update_metadata',
        )
        return False
    ...
    if scope.is_unscoped():
        ...
        if scope.update_metadata:
            metadata_files = db.op.get_metadata_file_list(
                directory=scope.target_directory
            )
        if scope.update_images:
        ...
        if scope.update_metadata or scope.update_images:
            ...
            seasons = db.op.get_show_season_list_by_show_id(
                ticket=ticket, show_id=scope.target_id
            )
            if scope.update_metadata or scope.update_images:
                for season in seasons:
                    if scope.update_metadata:
                        metadata_files += prep(
                            files=season.metadata_files, show_season=season
                        )
                    if scope.update_images:
                        image_files += prep(
                            files=season.image_files, show_season=season
                        )
        ...
        for episode in episodes:
            if scope.update_metadata:
                metadata_files = list(metadata_files) + prep(
                    files=episode.metadata_files, show_episode=episode
                )
            if scope.update_images:
                image_files = list(image_files) + prep(
                    files=episode.image_files, show_episode=episode
                )
            if scope.update_videos:
    ...
    if scope.update_metadata and metadata_files:
        db.op.update_job(
            job_id=scope.job_id,
            message=f'Updating {len(metadata_files)} metadata files',
        )
        defined_tag_ids = {}
        progress_count = 0
        updated_shows = {}
        ...
        for metadata_file in metadata_files:
            ...
            if progress_count % 500 == 0:
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'Read metadata file {progress_count} out of {len(metadata_files)}',
                )
            ...
            if not os.path.exists(metadata_file.local_path):
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'WARNING A metadata_file db entry exists for a path that does not exist\n\t({metadata_file.id})[{metadata_file.local_path}]',
                )
                continue
            try:
                nfo_content = snow_media.nfo.nfo_path_to_dict(
                    nfo_path=metadata_file.local_path
                )
                ...
                for tag_name in nfo_content['tag']:
                    ...
                    if not tag_name in defined_tag_ids:
                        ...
                        db.op.update_job(
                            job_id=scope.job_id,
                            message=f'Processed [{tag.name}] for the first time on {metadata_file.local_path}',
                        )
                    ...
                    if metadata_file.movie != None:
                        ...
                        db.op.upsert_show_tag(
                            show_id=metadata_file.show.id, tag_id=tag_id
                        )
                        if (
                            not metadata_file.show.release_year
                            and 'year' in nfo_content
                            and nfo_content['year']
                        ):
                            if not metadata_file.show.id in updated_shows:
                                update_show_years.append(
                                    [
                                        metadata_file.show.id,
                                        metadata_file.show.name,
                                        nfo_content['year'],
                                    ]
                                )
                                updated_shows[metadata_file.show.id] = True
                    elif metadata_file.show_season != None:
                        db.op.upsert_show_season_tag(
                            metadata_file.show_season.id, tag_id
                        )
                    elif metadata_file.show_episode != None:
                        db.op.upsert_show_episode_tag(
                            metadata_file.show_episode.id, tag_id
                        )
                    else:
                        db.op.update_job(
                            job_id=scope.job_id,
                            message=f'Did not find any content to upsert for tag {tag_name} on metadata_file {metadata_file.local_path}',
                        )
            except Exception as e:
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'An error occurred while processing metadata_file [{metadata_file.local_path}]',
                )
                import traceback

                db.op.update_job(
                    job_id=scope.job_id, message=f'{traceback.format_exc()}'
                )
        if update_show_years:
            for info in update_show_years:
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'Update show [{info[0]}][{info[1]}] release year to {info[2]}',
                )
                db.op.update_show_release_year(show_id=info[0], release_year=info[2])

    if scope.update_images and image_files:
        db.op.update_job(
            job_id=scope.job_id, message=f'Updating {len(image_files)} image files'
        )
        progress_count = 0
        for image_file in image_files:
            ...
            if progress_count % 500 == 0:
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'Read image file {progress_count} out of {len(image_files)}',
                )
            ...
            if not os.path.exists(image_file.local_path):
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'WARNING An image_file db entry exists for a path that does not exist\n\t({image_file.id})[{image_file.local_path}]',
                )
                continue
            try:
                snow_media.image.create_thumbnail(
                    local_path=image_file.local_path,
                    force_overwrite=(not scope.skip_existing),
                )
            except Exception as e:
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'An error occurred while processing image_file [{image_file.local_path}]',
                )
                import traceback
    ...
    if scope.update_videos and video_files:
        ...
        for video_file in video_files:
            ...
            try:
                ...
                db.op.update_job(
                    job_id=scope.job_id,
                    message=f'An error occurred while processing video_file [{video_file.local_path}]',
                )
                import traceback
