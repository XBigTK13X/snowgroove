...
def handle(scope: JobMediaScope):
    db.op.update_job(
        job_id=scope.job_id, message=f'[WORKER] Handling a delete_media_records job'
    )
    ...
    if not scope or scope.is_unscoped():
        db.op.update_job(
            job_id=scope.job_id, message='delete_media_records must be scoped when run'
        )
        return False
    ...
    if scope.is_shelf():
        db.op.update_job(
            job_id=scope.job_id,
            message='delete_media_records cannot delete an entire shelf',
        )
        return False
        ...
        db.op.delete_show_episode_records(
            ticket=ticket, show_episode_id=scope.target_id
        )

    else:
        db.op.update_job(
            job_id=scope.job_id, message='There is nothing for the job to do'
        )
