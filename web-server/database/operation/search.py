...
def perform_search(ticket: dbi.dm.Ticket, query: str):
    ...
    with dbi.session() as db:
        ...
        for shelf in shelves:
            if shelf.kind == 'Movies':
                movies = db_movie.get_movie_list(
                    ticket=ticket, shelf_id=shelf.id, search_query=query
                )
                if movies:
                    movie_results += movies
            elif shelf.kind == 'Shows':
                shows = db_show.get_show_list_by_shelf(
                    ticket=ticket, shelf_id=shelf.id, search_query=query
                )
                if shows:
                    show_results += shows
        ...
        if movie_results:
            ...
            results.append({'kind': 'movies', 'name': 'Movies', 'items': movie_results})
        if show_results:
            show_results.sort(key=lambda xx: len(xx.name))
            results.append({'kind': 'shows', 'name': 'Shows', 'items': show_results})
        if episode_results:
            episode_results.sort(key=lambda xx: len(xx.name))
            results.append(
                {'kind': 'episodes', 'name': 'Episodes', 'items': episode_results}
            )

        streamables = db_streamable.get_streamable_list(
            ticket=ticket, search_query=query
        )
        if streamables:
            streamables.sort(key=lambda xx: len(xx.name))
            results.append(
                {'kind': 'streamables', 'name': 'Streamables', 'items': streamables}
            )
